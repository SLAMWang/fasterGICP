...
void FeatureAssociation::laserCloudInfoHandler(const cloud_info *msgIn)
{
    segInfo = *msgIn;
...
//        if (dis_min>pow(point.x,2)+pow(point.y,2)+pow(point.z,2))
//            dis_min=pow(point.x,2)+pow(point.y,2)+pow(point.z,2);
        ...
        if (!halfPassed) {
            ...
            if (ori - segInfo.startOrientation > M_PI) {
                halfPassed = true;
//                cout<<"halfPassed "<<i<<" "<<cloudSize<<" "<<segInfo.startOrientation<<" "<<segInfo.orientationDiff<<endl;
...
//            cout<<halfPassed<<" "<<ori_<<" "<<ori<<" "<<" "<<segInfo.startOrientation<<" "<<segInfo.orientationDiff<<endl;
        point.intensity = int(segmentedCloud->points[i].intensity) + scanPeriod * relTime;
//        if (10 * (point.intensity - int(point.intensity))>1)
//            cout<<"intensity "<<10 * (point.intensity - int(point.intensity))<<endl;
...
//    cout<<"segmentedCloud: "<<segmentedCloud->size()<<endl;
//    cout<<"Surf Points: "<<surfPointsFlat->size()<<" "<<surfPointsLessFlat->size()<<endl;
//    cout<<"Corner Points: "<<cornerPointsSharp->size()<<" "<<cornerPointsLessSharp->size()<<endl;
}
    ...
    for (int i = 0; i < cornerPointsSharpNum; i++) {
        ...
        if (pointSearchCornerInd2[i] >= 0) {
            ...
            if (s > 0.1 && ld2 != 0) {
                ...
                cornor_point_ld2s.push_back(s*ld2);
...
//        if (iterCount % 5 == 0)
        ...
        if (pointSearchSurfInd2[i] >= 0 && pointSearchSurfInd3[i] >= 0) {
            ...
            if (s > 0.1 && pd2 != 0) {
                ...
                surf_point_pd2s.push_back(s*pd2);
...
//    cout<<matX<<endl;
    ...
    if  (isDegenerate) cout<<isDegenerate<<endl;
...
void FeatureAssociation::updateTransformation(){
    ...
    //cout<<transformCur[0]<<" "<<transformCur[1]<<" "<<transformCur[2]<<" "<<transformCur[3]<<" "<<transformCur[4]<<" "<<transformCur[5]<<endl;

    for (int iterCount1 = 0; iterCount1 < iterNum; iterCount1++) {
        laserCloudOri->clear();
        coeffSel->clear();

        surf_point_pd2s.clear();
...
//        cout<<transformCur[0]<<" "<<transformCur[1]<<" "<<transformCur[2]<<" "<<transformCur[3]<<" "<<transformCur[4]<<" "<<transformCur[5]<<endl;
    ...
    for (int iterCount2 = 0; iterCount2 < iterNum; iterCount2++) {

        laserCloudOri->clear();
        coeffSel->clear();

        cornor_point_ld2s.clear();
...
//    cout<<"Cloud_size: "<<segmentedCloud->size()<<endl;

//        if (newSegmentedCloud && newSegmentedCloudInfo && newOutlierCloud &&
//            std::abs(timeNewSegmentedCloudInfo - timeNewSegmentedCloud) < 0.05 &&
//            std::abs(timeNewOutlierCd - timeNewSegmentedCloud) < 0.05){
//
//            newSegmentedCloud = false;
//            newSegmentedCloudInfo = false;
//            newOutlierCloud = false;
